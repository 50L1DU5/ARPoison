...
pub fn send_arp_loop<'a>(interface: &'a datalink::NetworkInterface,
    ...
    let (mut tx, _) = match datalink::channel(interface, Default::default()){
        ...
        Err(_) => {
            eprintln!("Unable to create datalink channel");
            eprintln!("You may need \"sudo\" or root access to send raw packets on this machine. Exiting.");
            exit(255);
        },
    };
    ...
    let mut ethernet_frame = match MutableEthernetPacket::new(&mut ethernet_buf) {
        Some(f) => f,
        None => {
            eprintln!("Could not create mutable ethernet frame.");
            exit(254);
        },
    };

    ethernet_frame.set_destination(MacAddr::new(0xb8, 0x27, 0xeb, 0x1f, 0x25, 0xe0));
